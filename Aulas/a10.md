...
Na implementação acima, a inserção e remoção no início da lista é bem barata, mas a inserção ou remoção no final exigem o percurso da lista toda, porque necessitamos encontrar o último ou penúltimo elementos.
No caso de se usar essa lista para implementar uma pilha, isso não é problema, porque em uma pilha as remoções e inserções são todas na mesma extremidade da lista.
Para a implementação de uma fila, entretanto, uma das duas operações principais teria um desempenho muito inferior à outra.

A inserção no final poderia ser otimizada mantendo no descritor um ponteiro para o último nodo, além de um ponteiro para o primeiro. O custo seria pequeno, tanto em memória quanto em complexidade para manter esse dado, mas o ganho em desempenho seria significativo.

Outra possibilidade seria manter somente o ponteiro para o último, e este, em vez de ter um ponteiro para NULL, teria um ponteiro para o primeiro.

Não é incomum manter no descritor o número de elementos na lista, principalmente se a lista oferece uma operação para obter esse dado, que, sem essa adição, necessita um percurso completo da lista para ser obtido. Esse valor poderia também otimizar algumas operação de acesso por posição.

Quando o descritor da lista contém somente um ponteiro para o primeiro elemento como acima, por vezes se elimina o descritor, usando um ponteiro para o primeiro elemento da lista para representá-la. A desvantagem dessa implementação é que esse ponteiro é mantido pelo usuário da lista, e deve ser alterado sempre que o primeiro elemento da lista for alterado.

Em alguns casos, se usa um nodo extra, alocado quando a lista é criada, e que permanece alocado enquanto a lista existir. Esse nodo, chamado sentinela, não contém um dado válido, e é usado ara simplificar algumas operações de manipulação da lista, já que pode-se considerar que a lista nunca está realmente vazia.

Para o caso de se usar a lista encadeada para implementar um deque, uma das 4 operações principais terá um desempenho ruim (a remoção do final da lista). Caso isso não seja aceitável, uma solução é usar uma lista duplamente encadeada.
